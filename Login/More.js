...
// ----------------------------------------------------------------------------------------
const deleteTask = id => db.collection('More').doc(id).delete()
.then(()=>{
    Swal.fire(
        'Finish!',
        'Delete!',
...
const getTask = (id) => db.collection('More').doc(id).get();
// ------------------- show on website ---------------------------------------------------
...
document.getElementById("upload").onclick = function(){
    ...
    uploadTask.on('state_change', function(snapshot){
        var progress = (snapshot.byMoreransferred / snapshot.totalBytes)*100;
